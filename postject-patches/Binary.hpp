...
class LIEF_API Binary : public LIEF::Binary {
  ...
  //! Add the given note and return the created entry (move version)
  Note& add(Note&& note);
  ...
  Section* add_move(Section&& section, bool loaded = true);
  ...
  Segment* add_move(Segment&& segment, uint64_t base = 0);
